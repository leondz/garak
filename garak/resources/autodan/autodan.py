...
from garak.resources.autodan.model_utils import AutoDanPrefixManager, load_conversation_template, check_for_attack_success
import argparse
import os
...
autodan_parser.add_argument("--num_steps", type=int, default=100, help="Number of steps to run generation")
autodan_parser.add_argument("--batch_size", type=int, default=64, help="Batch size")
autodan_parser.add_argument("--num_elites", type=float, default=20,
                            help="Number of parents to pass unmutated to the next generation")
autodan_parser.add_argument("--crossover_rate", type=float, default=0.5,
                            help="Probability of performing crossover")
autodan_parser.add_argument("--num_points", type=int, default=5,
                            help="Number of points to perform crossover")
autodan_parser.add_argument("--mutation", type=float, default=0.01, help="Probability of mutation")
autodan_parser.add_argument("--init_prompt_path", type=str,
                            default=f"{Path(__file__).parents[0]}/data/autodan_init.txt", help="Path to initial prompt")
autodan_parser.add_argument("--reference", type=str,
                            default=f"{Path(__file__).parents[0]}/data/prompt_group.pth", help="Path to refernces")
autodan_parser.add_argument("--low_memory", action="store_true", help="Use low memory mode")
...
def autodan_generate(generator: Generator, prompt: str, target: str, num_steps: int = 100,
    ...
    for o in range(len(reference)):
        ...
        reference[o] = reference[o].replace('chatGPT', conv_template.name)
        reference[o] = reference[o].replace('ModelKeeper', developer_dict.get(conv_template.name, "Moderation"))
